...
export const Route = createFileRoute("/app/add")({
  head: () => ({ meta: [{ title: "Add Trade — PipLog" }] }),
...
function AddTradePage() {
  ...
  // Effective P/L: manual override if enabled, else auto-calculated
  ...
  const reset = () => {
    setForm({ ...initialForm, openedAt: new Date().toISOString().slice(0, 16) });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      navigate({ to: "/app/trades/$tradeId", params: { tradeId: data.id } });
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="px-4 pt-6 sm:px-6 lg:px-8 lg:pt-10">
      <header className="mx-auto max-w-6xl">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Log every detail. P/L is auto-calculated from your entry, exit and lot size.
        </p>
      </header>

      {/* Live summary strip — always visible at top on mobile */}
          ...
          <Section title="Trade Details" icon={Activity} description="Pair, direction, lot size, prices">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Lot size">
                <NumInput value={form.lotSize} onChange={(v) => setForm((f) => ({ ...f, lotSize: v }))} placeholder="0.10" />
              </Field>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Entry price">
                <NumInput value={form.entry} onChange={(v) => setForm((f) => ({ ...f, entry: v }))} placeholder="1.08500" />
              </Field>
              <Field label="Exit price">
                <NumInput value={form.exit} onChange={(v) => setForm((f) => ({ ...f, exit: v }))} placeholder="1.08750" />
              </Field>
            </div>

            {/* Auto P/L preview block */}
          ...
          <Section title="Risk Management" icon={Shield} description="Stops, targets, R:R and exposure">
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Stop loss">
                <NumInput value={form.sl} onChange={(v) => setForm((f) => ({ ...f, sl: v }))} placeholder="1.08300" />
              </Field>
              <Field label="Take profit">
                <NumInput value={form.tp} onChange={(v) => setForm((f) => ({ ...f, tp: v }))} placeholder="1.09000" />
              </Field>
              <Field label="Risk %">
                <NumInput value={form.riskPercent} onChange={(v) => setForm((f) => ({ ...f, riskPercent: v }))} placeholder="1.0" />
              </Field>
          ...
          <Section title="Strategy" icon={Lightbulb} description="What was the setup and context?">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Field label="Session">
                <Select
                  value={form.session}
                  onValueChange={(v) => setForm((f) => ({ ...f, session: v as typeof form.session }))}
                >
                  <SelectTrigger className="h-11 rounded-xl bg-input border-border">
                    <SelectValue />
                  </SelectTrigger>
          ...
          <div className="sticky top-6 space-y-4">
            ...
            <div className="surface-card rounded-2xl p-4">
              ...
              <ul className="mt-3 space-y-1.5 text-xs text-muted-foreground">
                <li>• Pip value uses standard $10/lot model.</li>
                <li>• Toggle <span className="font-semibold text-foreground">Override</span> to type the exact broker P/L.</li>
                <li>• Win/Loss is auto-detected from entry vs. exit.</li>
                <li>• Leave exit empty to log an open trade.</li>
              </ul>
            </div>
          </div>
        </aside>
